function Simulation(clientID,sim,Handle,fis,fisNO,simTime)
...
while elapsedTime<=simTime
    ...
    while it<4
        ...
        if SensorDetectionState(1,1) == 0 && SensorDetectionState(1,2) == 0 && SensorDetectionState(1,3) == 0
            ...
            HAvel=evalfis(fisNO,HA);

            if  Distance(1,:)>1
                sim.simxSetJointTargetVelocity(clientID,MotorHandle(1,1),HAvel(1,2),sim.simx_opmode_oneshot);
                sim.simxSetJointTargetVelocity(clientID,MotorHandle(1,2),HAvel(1,1),sim.simx_opmode_oneshot);
            else
                sim.simxSetJointTargetVelocity(clientID,MotorHandle(1,1),HAvel(1,2)*Distance(1,:),sim.simx_opmode_oneshot);
                sim.simxSetJointTargetVelocity(clientID,MotorHandle(1,2),HAvel(1,1)*Distance(1,:),sim.simx_opmode_oneshot);
            end
            ...
            VelM=evalfis(fis,OD(1,:));



            sim.simxSetJointTargetVelocity(clientID,MotorHandle(1,1),VelM(1,1),sim.simx_opmode_oneshot);
            sim.simxSetJointTargetVelocity(clientID,MotorHandle(1,2),VelM(1,2),sim.simx_opmode_oneshot);


        end

        if Distance(1,:)<0.2 &&  Distance(1,:)>0.1
            itr=0;
            break
        end
        ...
        if double(RobotPosition(1,1))~=0 && double(RobotPosition(1,2))~=0
            ...
            addpoints(h1,double(RobotPosition(1,2)),double(RobotPosition(1,1)));
